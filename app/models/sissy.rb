class Sissy < ApplicationRecord
  ...
  has_one    :shoes, -> { joins(:clothe).where(clothes: {kind: 'shoes'}) }, class_name: 'SissyClothe'
  has_one    :outfit, -> { joins(:clothe).where(clothes: {kind: 'outfit'}) }, class_name: 'SissyClothe'
  has_one    :headgear, -> { joins(:clothe).where(clothes: {kind: 'headgear'}) }, class_name: 'SissyClothe'

  # === Scopes ===
  scope :prostitutes, -> { joins(sissy_jobs: :job).where(sissy_jobs: { current_job: true }, jobs: { name: 'Prostitute' } ) }
  scope :maids,       -> { joins(sissy_jobs: :job).where(sissy_jobs: { current_job: true }, jobs: { name: 'Maid' } ) }
